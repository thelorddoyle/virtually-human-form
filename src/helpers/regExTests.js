...
export const phoneTest = new RegExp(/^(?:\+?(61))? ?(?:\((?=.*\)))?(0?[2-57-8])\)? ?(\d\d(?:[- ](?=\d{3})|(?!\d\d[- ]?\d[- ]))\d\d[- ]?\d[- ]?\d{3})$/);

// this checks if the field is ONLY space characters
export const spaceTest = new RegExp(/.*[^ ].*/);

// this checks for spaces at the beginning and end of the field
export const startAndEndTest = new RegExp(/^\S+(?: \S+)*$/);

// This is looking for a valid name, without numbers in etc
export const nameTest = new RegExp(/^[a-z ,.'-]+$/i);